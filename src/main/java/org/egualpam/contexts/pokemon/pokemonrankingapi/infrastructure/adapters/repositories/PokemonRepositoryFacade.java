...
public final class PokemonRepositoryFacade implements AggregateRepository<Pokemon> {
    ...
    private final Supplier<List<PokemonDTO>> pokemonDataSupplier;

    public PokemonRepositoryFacade(Supplier<List<PokemonDTO>> pokemonDataSupplier) {
        this.pokemonDataSupplier = pokemonDataSupplier;
    }
    ...
    private List<Pokemon> findSortedByHeight(Limit limit) {
        return pokemonDataSupplier.get().stream()
